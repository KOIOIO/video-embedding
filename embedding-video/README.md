...
- `video-service/`：推荐部署的 Go HTTP 视频服务，提供上传、转码、播放、推荐、观看记录和异步 worker。
...
└── embedding-video/
    ...
    ├── two-tower-training/             # 双塔推荐训练代码与模型产物
...
当 `FFmpeg.UseDocker=true` 时，服务容器会通过宿主机 Docker daemon 启动 ffmpeg/ffprobe 子容器。`docker run -v` 的源路径由宿主机解析，因此 compose 会把项目目录挂载到容器内的同名绝对路径。
