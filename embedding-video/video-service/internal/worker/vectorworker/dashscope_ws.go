...
func dashscopeWSRecognizeWav(ctx context.Context, wsURL string, apiKey string, model string, audioPath string) (string, error) {
	...
	if wsURL == "" {
		wsURL = config.ASRWSURL(config.Config{})
	}
	...
	waitStarted := func() error {
		...
		for {
			...
			case "task-failed":
				...
				if strings.TrimSpace(e.Header.ErrorCode) != "" && strings.TrimSpace(e.Header.ErrorMessage) != "" {
					return errors.New(e.Header.ErrorCode + ": " + e.Header.ErrorMessage)
				}
				if strings.TrimSpace(e.Header.ErrorCode) != "" {
					return errors.New(e.Header.ErrorCode)
				}
