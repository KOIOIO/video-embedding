...
func Register(app *lifecycle.App, cfg config.Config) {
	...
	if err != nil {
		if shouldSkipVectorWorker(err) {
			zap.L().Warn("vector_worker_disabled",
				...
				zap.String("hint", "set DASHSCOPE_API_KEY or OPENAI_API_KEY to enable vector worker"))
			return
		}
	...
	embeddingDim := config.EmbeddingDim(cfg)
	...
	tmpRoot := config.VectorTempPath(cfg)
	_ = os.MkdirAll(tmpRoot, 0755)

	queueKey := config.VectorizeQueueKey(cfg)
	...
	if strings.EqualFold(mode, "hierarchical") {
		prepareHandler := newPrepareStageHandlerWithRefine(
		...
		coarseHandler := newCoarseStageHandler(
			stageRepo,
			newProductionCoarseStageProcessor(store, ff, client, tmpRoot, sampleASRPoolSize, coarsePoolSize, stageRecorder),
			stageQueues.queue(VectorStageRefine),
		)
		refineHandler := newRefineStageHandler(
			newGormRefineStageRepository(db, stageRepo),
			newProductionRefineStageProcessor(db, store, ff, client, tmpRoot, coarseSegmentSec, refineMinSegmentSec, refineMaxSegmentSec, llmModel, llmTimeoutMinutes, sampleASRPoolSize, embedBatch, embeddingDim, tailCfg, stageRecorder),
			stageQueues.queue(VectorStageFinalize),
		)
		finalizeHandler := newFinalizeStageHandler(stageRepo)
		stageHandlers := map[string]vectorStageHandler{
			VectorStagePrepare:  prepareHandler,
			VectorStageCoarse:   coarseHandler,
			VectorStageRefine:   refineHandler,
			VectorStageFinalize: finalizeHandler,
		}
		for _, stage := range vectorStageOrder {
			stageName := stage
			stageQueue := stageQueues.queue(stageName)
			stageHandler := stageHandlers[stageName]
			for i := 0; i < vectorStageWorkerCountFromConfig(cfg, stageName); i++ {
				app.Go(func(ctx context.Context) error {
					return runVectorStageWorker(ctx, stageName, stageQueue, stageHandler)
				})
			}
		}
	}
		...
		app.Go(func(ctx context.Context) error {
			for {
				...
				for retry := 0; retry < maxRetryTimes; retry++ {
					...
					handledErr := func() error {
						videoapp.RuntimeCounters().Inc("vector_tasks_active")
						defer videoapp.RuntimeCounters().Dec("vector_tasks_active")
						if strings.EqualFold(mode, "hierarchical") {
							return stageAdapter.Handle(taskCtx, task)
						}
						return handleVectorizeTask(taskCtx, db, store, ff, client, tmpRoot, mode, windowSec, stepSec, sampleASRPoolSize, coarsePoolSize, embedBatch, sampleCount, sampleDurSec, coarseSegmentSec, refineMinSegmentSec, refineMaxSegmentSec, llmModel, llmTimeoutMinutes, embeddingDim, tailCfg, stageRecorder, task.VideoID, task.TaskID, task.RawKey)
					}()
					lastErr = handledErr
					if cancel != nil {
						cancel()
					...
					if lastErr == nil {
						zap.L().Info("vector_task_done",
							zap.Int("worker_id", id),
							zap.Uint64("video_id", task.VideoID),
							zap.String("task_id", task.TaskID),
							zap.Int("retry", retry),
							zap.Int64("latency_ms", time.Since(taskStart).Milliseconds()),
						)
						break
					}
					...
					if !decision.Retry {
						zap.L().Error("vector_task_failed",
							zap.Int("worker_id", id),
							zap.Uint64("video_id", task.VideoID),
							zap.String("task_id", task.TaskID),
					...
					if retry < maxRetryTimes-1 {
						...
						select {
						case <-time.After(maxDuration(retryDelay, decision.Delay)):
							retryDelay *= 2
						case <-ctx.Done():
							return nil
						}
...
func shouldSkipVectorWorker(err error) bool {
	return errors.Is(err, errMissingOpenAICompatAPIKey)
}
