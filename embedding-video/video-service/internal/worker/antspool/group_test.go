...
func TestGroupMetricsRecorderReceivesLifecycleCallbacks(t *testing.T) {
	...
	g, err := New(context.Background(), Options{Name: "vector.refine_asr", Size: 2, Recorder: recorder})
	if err != nil {
		t.Fatalf("New() error = %v", err)
	}
	defer g.Release()
	...
	if recorder.submitted != 2 || recorder.taskDone != 1 || recorder.taskErrs != 1 || recorder.submitErrs != 0 {
		t.Fatalf("unexpected recorder counts: submitted=%d done=%d errs=%d submitErrs=%d", recorder.submitted, recorder.taskDone, recorder.taskErrs, recorder.submitErrs)
	}
}
