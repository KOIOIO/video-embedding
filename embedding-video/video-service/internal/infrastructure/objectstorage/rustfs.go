...
// Open 获取对象读取句柄，供 HTTP 代理层按 io.ReadCloser 抽象读取对象内容。
func (s *RustFS) Open(ctx context.Context, objectKey string, opts minio.GetObjectOptions) (io.ReadCloser, error) {
	return s.Get(ctx, objectKey, opts)
}
