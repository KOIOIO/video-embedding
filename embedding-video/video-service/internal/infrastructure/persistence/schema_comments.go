...
var schemaCommentDefinitions = []tableCommentDefinition{
	{
		...
		Columns: []columnCommentDefinition{
			...
			columnComment("source_type", "视频来源类型：admin_import 管理员导入，user_publish 用户发布"),
	...
	{
		Schema:  "public",
		Table:   "edu_video_comment",
		Comment: "视频评论表",
		Columns: []columnCommentDefinition{
			columnComment("id", "评论主键 ID"),
			columnComment("user_id", "评论用户 ID"),
			columnComment("video_segment_id", "关联视频片段 ID"),
			columnComment("root_id", "根评论 ID，0 表示根评论"),
			columnComment("parent_id", "父评论 ID，0 表示无父评论"),
			columnComment("reply_to_user_id", "回复目标用户 ID，0 表示无"),
			columnComment("content", "评论内容"),
			columnComment("like_count", "点赞次数"),
			columnComment("double_like_count", "双击点赞次数"),
			columnComment("dislike_count", "点踩次数"),
			columnComment("create_time", "创建时间"),
	...
	{
		Schema:  "public",
		Table:   "edu_comment_like",
		Comment: "评论点赞记录表",
		Columns: []columnCommentDefinition{
			columnComment("id", "点赞记录主键 ID"),
			columnComment("comment_id", "关联评论 ID"),
			columnComment("user_id", "点赞用户 ID"),
			columnComment("reaction_type", "互动类型：like 点赞"),
			columnComment("create_time", "创建时间"),
			columnComment("update_time", "更新时间"),
			columnComment("deleted", "逻辑删除标记：0 未删除，非 0 已删除"),
		},
	},
	{
		...
		Comment: "用户资料表",
		Columns: []columnCommentDefinition{
			columnComment("id", "资料主键 ID"),
			columnComment("user_id", "用户 ID"),
			columnComment("nickname", "昵称"),
			columnComment("avatar_url", "头像访问地址"),
			columnComment("bio", "个人简介"),
			columnComment("gender", "性别：0 未知，1 男，2 女"),
			columnComment("location", "所在地"),
			columnComment("follow_count", "关注数"),
			columnComment("fans_count", "粉丝数"),
			columnComment("create_time", "创建时间"),
			columnComment("update_time", "更新时间"),
			columnComment("deleted", "逻辑删除标记：0 未删除，非 0 已删除"),
		},
	...
	{
		Schema:  "public",
		Table:   "edu_user_follow",
		Comment: "用户关注关系表",
		Columns: []columnCommentDefinition{
			columnComment("id", "关注关系主键 ID"),
			columnComment("follower_id", "关注者用户 ID"),
			columnComment("following_id", "被关注者用户 ID"),
			columnComment("create_time", "创建时间"),
			columnComment("deleted", "逻辑删除标记：0 未删除，非 0 已删除"),
		},
	},
	{
		Schema:  "public",
		Table:   "edu_user_message",
		Comment: "用户私信消息表",
		Columns: []columnCommentDefinition{
			columnComment("id", "消息主键 ID"),
			columnComment("sender_id", "发送者用户 ID"),
			columnComment("receiver_id", "接收者用户 ID"),
			columnComment("conversation_id", "会话 ID"),
			columnComment("content", "消息内容"),
			columnComment("is_read", "是否已读"),
			columnComment("create_time", "创建时间"),
			columnComment("deleted", "逻辑删除标记：0 未删除，非 0 已删除"),
		},
	},
	{
		Schema:  "public",
		Table:   "edu_user_profile_visit",
		Comment: "用户主页访问记录表",
		Columns: []columnCommentDefinition{
			columnComment("id", "访问记录主键 ID"),
			columnComment("visitor_id", "访问者用户 ID"),
			columnComment("owner_id", "被访问者用户 ID"),
			columnComment("visit_date", "访问日期"),
			columnComment("visit_count", "当日访问次数"),
			columnComment("create_time", "创建时间"),
			columnComment("update_time", "更新时间"),
			columnComment("deleted", "逻辑删除标记：0 未删除，非 0 已删除"),
		},
	...
	{
		Schema:  "public",
		Table:   "edu_user_notification",
		Comment: "用户通知表",
		Columns: []columnCommentDefinition{
			columnComment("id", "通知主键 ID"),
			columnComment("user_id", "接收通知用户 ID"),
			columnComment("type", "通知类型：mention 评论 @提及"),
			columnComment("from_user_id", "触发通知的用户 ID"),
			columnComment("video_id", "关联视频 ID"),
			columnComment("video_segment_id", "关联视频片段 ID"),
			columnComment("comment_id", "关联评论 ID"),
			columnComment("content", "通知内容"),
			columnComment("is_read", "是否已读：0 未读，1 已读"),
			columnComment("create_time", "创建时间"),
			columnComment("update_time", "更新时间"),
			columnComment("deleted", "逻辑删除标记：0 未删除，非 0 已删除"),
		},
