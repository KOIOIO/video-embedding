...
**Goal:** 在 `video-embedding` 的 hierarchical 分段链路中新增一个可降级的 `Boundary Alignment` 层，同时提升 LLM 语义边界表达能力和 ASR 边界对齐能力，降低开头半句、结尾半句和知识点硬裁切问题。
...
- `video-embedding/internal/worker/vectorworker/tasks/db.go`
  - 扩展 `LLMSegment`，增加可选辅助边界字段，保持现有入库字段兼容。
- `video-embedding/internal/worker/vectorworker/tasks/hierarchical.go`
  - 更新主 prompt / retry prompt，对 LLM 提出 `boundary_reason`、`start_anchor_text`、`end_anchor_text`、`boundary_confidence` 要求。
- `video-embedding/internal/worker/vectorworker/tasks/hierarchical_test.go`
  - 新增 prompt 和 JSON normalization 的测试，约束新增字段和兼容解析行为。
- `video-embedding/internal/worker/vectorworker/tasks/tail_alignment.go`
  - 保留现有尾部规则，抽出共享句首/句尾判断辅助函数，供新边界对齐器复用。
- `video-embedding/internal/worker/vectorworker/tasks/boundary_alignment.go`
  - 新增边界对齐引擎：窗口计算、候选打分、起止点校正、相邻片段重叠限制。
- `video-embedding/internal/worker/vectorworker/tasks/boundary_alignment_test.go`
  - 新增规则级测试，覆盖开头半句、结尾半句、重叠限制、锚点命中和降级路径。
- `video-embedding/internal/worker/vectorworker/tasks/asr.go`
  - 在 refine 阶段接入新的边界对齐器，保留失败时回退到 `alignSegmentTail` 的路径。
- `video-embedding/internal/worker/vectorworker/tasks/asr_boundary_alignment_test.go`
  - 新增 refine 集成测试，约束校正结果和降级行为。
- `video-embedding/docs/superpowers/specs/2026-05-11-llm-boundary-alignment-design.md`
...
**Files:**
- Modify: `video-embedding/internal/worker/vectorworker/tasks/db.go`
- Modify: `video-embedding/internal/worker/vectorworker/tasks/hierarchical.go`
- Create: `video-embedding/internal/worker/vectorworker/tasks/hierarchical_test.go`

- [ ] **Step 1: 先写 JSON normalization 兼容测试**

创建 `video-embedding/internal/worker/vectorworker/tasks/hierarchical_test.go`，先锁定“新增字段可解析、缺失字段不报错、未知字段被忽略”的行为：
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -run "TestNormalizeLLMSegments(KeepsOptionalBoundaryFields|AllowsMissingOptionalBoundaryFields)" -v`
...
修改 `video-embedding/internal/worker/vectorworker/tasks/db.go`：
...
修改 `video-embedding/internal/worker/vectorworker/tasks/hierarchical.go` 的 `NormalizeLLMSegments` 循环，补充字符串 trim 和置信度规范化：
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -run "Test(BuildHierarchicalSegmentationPromptMentionsBoundaryFields|NormalizeLLMSegments)" -v`
...
修改 `video-embedding/internal/worker/vectorworker/tasks/hierarchical.go`，在 `BuildHierarchicalSegmentationPrompt` 和 `BuildHierarchicalSegmentationRetryPrompt` 中新增这些约束文本：
...
```bash
git add video-embedding/internal/worker/vectorworker/tasks/db.go video-embedding/internal/worker/vectorworker/tasks/hierarchical.go video-embedding/internal/worker/vectorworker/tasks/hierarchical_test.go
...
**Files:**
- Modify: `video-embedding/internal/worker/vectorworker/tasks/tail_alignment.go`
- Modify: `video-embedding/internal/worker/vectorworker/tasks/tail_alignment_test.go`

- [ ] **Step 1: 先写句首/句尾规则测试**

在 `video-embedding/internal/worker/vectorworker/tasks/tail_alignment_test.go` 追加：
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -run "Test(LooksLikeSentenceStart|NormalizeBoundaryConfidence)" -v`
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -run "Test(NormalizeTailAlignmentConfigDefaults|LooksLikeSentenceEnd|NeedsTailExtension|NextAlignedEndSec|LooksLikeSentenceStart|NormalizeBoundaryConfidence)" -v`
...
```bash
git add video-embedding/internal/worker/vectorworker/tasks/tail_alignment.go video-embedding/internal/worker/vectorworker/tasks/tail_alignment_test.go video-embedding/internal/worker/vectorworker/tasks/hierarchical.go
...
**Files:**
- Create: `video-embedding/internal/worker/vectorworker/tasks/boundary_alignment.go`
- Create: `video-embedding/internal/worker/vectorworker/tasks/boundary_alignment_test.go`

- [ ] **Step 1: 先写规则级测试，约束窗口、重叠和锚点行为**

创建 `video-embedding/internal/worker/vectorworker/tasks/boundary_alignment_test.go`：
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -run "Test(BuildBoundaryWindows|AlignSegmentBoundariesPrefersNaturalSentenceAndCapsOverlap|ScoreBoundaryCandidateUsesAnchorAndConfidence)" -v`
...
创建 `video-embedding/internal/worker/vectorworker/tasks/boundary_alignment.go`：
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -run "Test(BuildBoundaryWindows|AlignSegmentBoundaries|ScoreBoundaryCandidate)" -v`
...
```bash
git add video-embedding/internal/worker/vectorworker/tasks/boundary_alignment.go video-embedding/internal/worker/vectorworker/tasks/boundary_alignment_test.go
...
**Files:**
- Modify: `video-embedding/internal/worker/vectorworker/tasks/asr.go`
- Create: `video-embedding/internal/worker/vectorworker/tasks/asr_boundary_alignment_test.go`
- Keep: `video-embedding/internal/worker/vectorworker/tasks/asr_tail_alignment_test.go`

- [ ] **Step 1: 先写 refine 集成测试，约束“优先新对齐器，失败回退旧 tail alignment”**

创建 `video-embedding/internal/worker/vectorworker/tasks/asr_boundary_alignment_test.go`：
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -run "TestAlignSegmentForRefine" -v`
...
在 `video-embedding/internal/worker/vectorworker/tasks/asr.go` 增加一个小型协调函数：
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -run "Test(AlignSegmentForRefine|AlignSegmentTail)" -v`
...
```bash
git add video-embedding/internal/worker/vectorworker/tasks/asr.go video-embedding/internal/worker/vectorworker/tasks/asr_boundary_alignment_test.go
...
**Files:**
- Modify: `video-embedding/internal/worker/vectorworker/tasks/asr.go`
- Modify: `video-embedding/internal/worker/vectorworker/tasks/boundary_alignment.go`
- Modify: `video-embedding/internal/worker/vectorworker/tasks/boundary_alignment_test.go`
- Modify: `video-embedding/docs/superpowers/specs/2026-05-11-llm-boundary-alignment-design.md` only if implementation reality diverges
...
Run: `go test ./video-embedding/internal/worker/vectorworker/tasks -v`
...
```bash
git add video-embedding/internal/worker/vectorworker/tasks/asr.go video-embedding/internal/worker/vectorworker/tasks/boundary_alignment.go video-embedding/internal/worker/vectorworker/tasks/boundary_alignment_test.go video-embedding/internal/worker/vectorworker/tasks/asr_boundary_alignment_test.go
