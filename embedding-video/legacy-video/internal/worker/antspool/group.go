...
// Options 控制 Group 的名称、大小和可选观测能力。
type Options struct {
	Name     string
	Size     int
	Logger   *zap.Logger
...
// Group wraps an ants pool with task tracking and first-error capture.
type Group struct {
	ctx      context.Context
	name     string
	pool     *ants.Pool
...
func New(ctx context.Context, opts Options) (*Group, error) {
	...
	if g.logger != nil {
		g.logger.Info("ants_pool_created",
			zap.String("pool_name", g.name),
...
func (g *Group) Submit(fn func() error) error {
	...
	err := g.pool.Submit(func() {
		...
		if err := fn(); err != nil {
			...
			if g.logger != nil {
				g.logger.Warn("ants_pool_task_failed",
					zap.String("pool_name", g.name),
	...
	if err != nil {
		...
		if g.logger != nil {
			g.logger.Warn("ants_pool_submit_failed",
				zap.String("pool_name", g.name),
				zap.Error(err),
			)
		}
...
func (g *Group) Wait() error {
	...
	if g.logger != nil {
		g.logger.Info("ants_pool_wait_done",
			zap.String("pool_name", g.name),
