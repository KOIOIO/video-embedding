...
func NormalizeLLMSegments(llmOut string, durationSec int, minSec int, maxSec int) ([]llmSegment, error) {
	...
	zap.L().Info("vectorize_hierarchical_llm_segments_raw",
	...
	for _, s := range parsed.Segments {
		...
		zap.L().Info("vectorize_hierarchical_llm_segment_raw",
	...
	for _, s := range tmp {
		...
		if shouldMergeLowConfidenceContinuation(*prev, s) {
			...
			zap.L().Info("vectorize_hierarchical_llm_segments_merge_continuation",
	...
	for i := range merged {
		...
		zap.L().Info("vectorize_hierarchical_llm_segment_normalized",
	...
	zap.L().Info("vectorize_hierarchical_llm_low_confidence_summary",
...
func shouldMergeLowConfidenceContinuation(prev llmSegment, curr llmSegment) bool {
	...
	for _, prefix := range continuationPrefixes {
		if anchor != "" && strings.HasPrefix(anchor, prefix) {
			return true
		}
		if summary != "" && strings.HasPrefix(summary, prefix) {
			return true
		}
	}
	if anchor == "" && strings.Contains(summary, "继续") {
		return true
	}
	_ = prev
	return false
